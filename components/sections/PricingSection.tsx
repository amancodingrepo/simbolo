...
const defaultPlans: PricingPlan[] = [
  {
    ...
    ctaHref: "/contact",
    description:
      "High-impact media assets to test offers and elevate your baseline aesthetic.",
    features: [
  ...
  },
  {
    ...
    ctaHref: "/contact",
    description:
      "An end-to-end media and marketing engine designed for aggressive market capture.",
    features: [
      ...
      <div className="container">
        ...
        <div className="row">
          {plans.map((plan, i) => (
            <div
              key={i}
              className={`col-lg-6 ${i < plans.length - 1 ? "lg-mb-24" : ""}`}
            >
              <div
              ...
              >
                <div className="top d-flex gap-12 align-items-center">
                  ...
                  <div
                    className={`fw-semibold ${plan.style === "black" ? "text-neutral-400" : "text-secondary"}`}
                  >
                    {plan.subtitle}
                  </div>
                </div>
                <div className="heading">
                  <div className="d-flex gap-14 align-items-end">
                    <div className="price-number fw-bold">
                      {formatPrice(
                        isAnnually ? plan.priceYear : plan.priceMonth,
                      )}
                    </div>
                    {formatPrice(
                      isAnnually ? plan.priceYear : plan.priceMonth,
                    ) !== "Custom" && (
                      <h6 className="price-per">
                        {isAnnually ? "/ retainer" : "/ project"}
                      </h6>
                    )}
                  </div>
