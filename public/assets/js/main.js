...
(function ($) {
    ...
    var counter = function () {
        // Remove class check because the body class might not be present or we can just rely on the element
        var a = 0;
    ...
    window.initSimboloMain = function() {
        infiniteSlide();
        updateClock();
        cursorTrail();
        goTop();
        settingColor();
        openMbMenu();
        // switchPrice(); // React state
        // services_btn(); // React state
        counter();
        dot();
        viewbox();
    };
