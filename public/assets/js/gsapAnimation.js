(function ($) {
    ...
    window.initSimboloGSAP = function() {
        ...
        if ($(".text-color-change").length) {
            $(".text-color-change").each(function () {
                const $el = $(this)[0];
                if ($el.charSplit) $el.charSplit.revert();
                if ($el.wordSplit) $el.wordSplit.revert();
            });
        }
    ...
    $(function () {
        if (typeof window.initSimboloGSAP === "function" && !$(".preloader").length) {
            loader();
