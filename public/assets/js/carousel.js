window.initSimboloSwipers = function () {
    ...
    $(".tf-swiper").each(function (index, element) {
        ...
        var getValidEl = function(els) {
            var valid = els.filter(Boolean);
            return valid.length > 0 ? valid[0] : null;
        };
        var getValidEls = function(els) {
            var valid = els.filter(Boolean);
        ...
        var swiperT = new Swiper($this[0], {
            ...
            pagination: {
                el: getValidEl([$this.find(".tf-sw-pagination")[0], $this.closest(".tf-pag-swiper").find(".tf-sw-pagination")[0]]),
                clickable: true,
            },
            observer: true,
            observeParents: true,
        ...
        window.SimboloSwiperInstances.push(swiperT);

        $this.find(".swiper-button")
    ...
    if ($(".section-testimonials").length > 0) {
        ...
        const mainSwiper = new Swiper(".swiper-testimonial", {
            slidesPerView: 1,
            pagination: { el: ".testimonials-pagination", type: "fraction" },
            navigation: { nextEl: ".testimonials-next", prevEl: ".testimonials-prev" },
        });

        if (thumbSwiper.controller && mainSwiper.controller) {
    ...
    if ($(".swiper-progressbar").length > 0) {
        ...
        window.SimboloSwiperInstances.push(swiper);
        ...
        function animateProgress(timestamp) {
            ...
            // Only continue rendering if Swiper still exists in DOM
            if (document.body.contains(progressFill)) {
...
$(window).on("load", function () {
    if (typeof window.initSimboloSwipers === "function") {
        window.initSimboloSwipers();
    }
});